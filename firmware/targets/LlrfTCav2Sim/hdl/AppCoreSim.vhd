...
architecture mapping of AppCoreSim is
   ...
   signal delayDataIn, delayDataOut : slv(70 downto 0) := (others=>'0');
...
--   end generate GEN_WAVEFORMS;
   ...
   U_DiagnBus : entity xil_defaultlib.AppDiagnBus
     generic map (
       FIFO_ADDR_WIDTH_G => 6 )  -- expect < 256 us delay in diagnBus
     port map (
       -- timingClk domain
       timingClk      => timingClk,
       timingRst      => timingRst,
   ...
   --
   --  Simulate the AppLlrfCore
   --
   U_DBDATA : entity surf.SlvDelay
     generic map (
       DELAY_G => 600,
       WIDTH_G => delayDataIn'length)
     port map (
       clk     => diagnClk,
       rst     => diagnRst,
       din     => delayDataIn,
   ...
   delayDataIn <= s_trigStrobe & s_trigIndex & trigTimeSlot & trigTimestamp;
   diagnBus.strobe <= delayDataOut(70);
   diagnBus.timingMessage.timeStamp <= delayDataOut(63 downto 0);
   diagnBus.data(0)(4 downto 0) <= delayDataOut(68 downto 64);
